 ...
 **SFX Density**: Pure Zero (Auditory experience driven entirely by BGM and Voiceovers).
 ...
 ### Blueprint C · The Fluid Office Narrative (Claude For Word Template)
 
 **Intended For**: Corporate suites, document editors, professional software.
 **Rhythm**: Dolly In/Out sweeps across segmented chapters.
 **Easings**: `overshoot` (Toggles) + `expoOut` (Panels).
 **SFX Density**: Mid-tier (~0.3/s) relying primarily on UI clicks.
 ...
 |---|---|---|
 | `transition: all 0.3s ease` | `ease` behaves almost linearly; breaks fluid physical reality limits. | Apply `expoOut` + individualized element staggering. |
 | `opacity 0→1` directly | Denies directional momentum. | Combine with `translateY 10→0` + Anticipation. |
 | Merely fading in a Logo | Lacks conclusive storytelling weight. | Morph / Converge / Collapse-Then-Expand. |
 | Linear Mouse Drags | Induces subconscious robot perception. | Bezier Arcs + subtle Perlin Noise. |
 | `setInterval` letter typing | Looks like legacy movie subtitles. | Chunk Reveal + randomized interval delays. |
 | Focus swaps relying on pure opacity | Non-focused elements remain painfully sharp. | Apply opacity + brightness + **blur(4-8px)**. |
 | Pure Black/White Backdrops | Induces cyber glare fatigue. | Add neutral warmth/coolness utilizing standard branding. |
 | Uniform Speeding | Lacks rhythm context entirely. | Adopt Slow-Fast-Boom-Stop framing. |
 | Fading out endings | Generates zero conclusion depth. | Halt abruptly (holding purely on the final frame). |
 
 ---
 
 ## 7. Self-Audit Checklist (60 Seconds Before Publishing)
 
 - [ ] Does the narrative follow Slow-Fast-Boom-Stop rather than uniform pacing?
 - [ ] Is the default easing `expoOut` instead of `ease` or `linear`?
 - [ ] Are Toggles/Popups utilizing `overshoot`?
 - [ ] Do lists/cards load utilizing a 30ms stagger?
 - [ ] Is there an explicit 0.5s pause before a critical AI result arrives?
 - [ ] Is typing simulating Chunk Reveals rather than linear 1-by-1 spans?
 - [ ] Did you inject `blur` into non-focused panels?
 - [ ] The logo physically morphs together, rather than purely sliding their opacity?
 - [ ] The backdrop relies on Off-Whites/Temperatures, never `#FFFFFF`?
 - [ ] The text hierarchy explicitly branches Serif parameters alongside Sans?
 - [ ] Ends via halting abruptly?
 - [ ] Are SFX densities actively matched against product personalities?
 - [ ] Is there a -6 to -8dB Loudness difference shielding SFX clear over BGM limits?
 
 ---
 
 ## 8. Relations to Other References
 
 | Reference | Position | Relationship |
 |---|---|---|
 | `animation-pitfalls.md` | Technical traps | "What NOT to do" |
 | `animations.md` | Core syntax usage | How to actually type code for it |
 | `audio-design-rules.md` | Dual-track sound policies | How to map audio identically |
 | `apple-gallery-showcase.md` | Specific style templates | Deep-dive onto specific frameworks |
 | **This File** | Positive Syntaxes | "What you SHOULD do" |
