...
(async () => {
  ...
  if (hasReady) {
    ...
    // This covers HTML files that do not strictly follow the starter tick template,
    // such as when lastTick uses performance.now() and font-loading time leaks into the first-frame delta.
    ...
    if (seekCorrected) {
      // Wait for two rAF cycles so seek applies and the t=0 frame renders.
